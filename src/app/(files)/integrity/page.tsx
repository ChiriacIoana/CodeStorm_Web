...
const reportRows: ReportRow[] = [
  {
    file: "Contract_Furnizor_A.docx",
    issues: "Clauza 4 · Secțiune GDPR · Anexa B",
    severity: "critic",
  },
  {
    file: "Fisa_Disciplina_BD.docx",
    issues: "Bibliografie · Secțiunea Obiective · Footer",
    severity: "critic",
  },
  {
    file: "Contract_Servicii_03.docx",
    issues: "Secțiune penalități · Clauza de confidențialitate",
    severity: "critic",
  },
  {
    file: "Fisa_Disciplina_ML.docx",
    issues: "Semnătură · Dată completare",
    severity: "avertisment",
  },
  {
    file: "Contract_Furnizor_B.docx",
    issues: "Număr înregistrare · Cod fiscal",
    severity: "avertisment",
  },
  {
    file: "Fisa_Disciplina_SO.docx",
    issues: "Toate câmpurile prezente",
    severity: "valid",
  },
];
...
const mathRows: MathRow[] = [
  ...
  },
  {
    label: "Proiect",
    value: 30,
    expected: "așteptat ≤30%",
    status: "ok",
    max: 30,
  },
  {
    label: "Activitate curs",
    value: 15,
    expected: "așteptat ≥20%",
    status: "mic",
    max: 20,
  },
];
    ...
    <div className="p-6 flex flex-col gap-5 max-w-4xl">
      ...
      <div className="flex flex-col gap-1">
        <div className="flex items-center justify-between">
          ...
          <GlowButton onClick={handleUploadFile} className="rounded-full px-4">
            <Plus size={18} />
            Upload
          </GlowButton>
        </div>
        ...
        <div className="grid grid-cols-3 gap-2.5">
          {[
            ...
            >
              ...
              <p
                className={`text-[22px] font-medium ${color || "text-zinc-900 dark:text-zinc-100"}`}
              >
                {value}
              </p>
            </div>
      ...
      {uc === "uc2" && (
        <div className="rounded-xl border border-zinc-200 dark:border-zinc-800 overflow-hidden bg-white dark:bg-zinc-900">
          ...
          <div className="px-4 pt-3 pb-1 flex flex-col gap-2.5">
            <div className="flex gap-2.5 items-start p-3 rounded-lg bg-red-50 border border-red-200">
              ...
              <p className="text-[12px] text-red-800 leading-relaxed">
                <span className="font-medium">Fisa_Disciplina_BD.docx</span> —
                Suma ponderilor de evaluare este 95%, nu 100%. Lipsesc 5%.
              </p>
            </div>
            <div className="flex gap-2.5 items-start p-3 rounded-lg bg-amber-50 border border-amber-200">
              ...
              <p className="text-[12px] text-amber-900 leading-relaxed">
                <span className="font-medium">Factura_Furnizor_A.pdf</span> —
                Total declarat 12.450 RON, suma pozițiilor: 11.980 RON.
                Diferență: 470 RON.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 px-4 py-3 border-t border-zinc-300 dark:border-zinc-700">
            ...
            <span className="text-[12px] text-red-600 w-28 text-right">
              trebuie 100%
            </span>
            <span
              className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${MATH_BADGE.eroare}`}
            >
              eroare
            </span>
          </div>
...
function ReportRowItem({ row }: { row: ReportRow }) {
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800 last:border-b-0">
      ...
      <span
        className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${SEVERITY[row.severity]}`}
      >
...
function MathRowItem({ row }: { row: MathRow }) {
  ...
  return (
    <div className="flex items-center gap-3 px-4 py-3 border-b border-zinc-100 dark:border-zinc-800">
      ...
      <span className="text-[12px] text-zinc-400 w-28 text-right">
        {row.expected}
      </span>
      <span
        className={`text-[11px] font-medium px-2.5 py-1 rounded-full ${MATH_BADGE[row.status]}`}
      >
        {row.status}
      </span>
    </div>
